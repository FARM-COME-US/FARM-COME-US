...
| 팀장, BE, 회원 인증 개발, 회원 및 스토어 API 담당 | BE 리더, 상품 및 라이브 API, CI/CD, 인프라, 배포 담당 | 팀원, BE, 결제 시스템 서버 및 클라이언트 개발 담당 | FE 리더, 디자인, 와이어프레임 설계, WebRTC 개발 | FE, 기획 및 화면 설계, UCC | FE, 화면 설계, UCC 담당 |
...
1. 라이브 커머스 중 목표 판매 재고를 실시간으로 확인 가능 (생동감 있는 라이브 판매 시각화)
